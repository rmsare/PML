...
def tiles_for_bounds(bounds, buffer=None, buffer_fraction=None):
    ...
    assert not (
        buffer is not None and buffer_fraction is not None
    ), "Keyword arguments buffer and buffer_fraction cannot be simultaneously specified"
    ...
    for y in range(0, num_y):
        for x in range(0, num_x):
            tiles += [
                (
                    (d_tile * x + offset_x, d_tile * (x + 1) + offset_x),
                    (d_tile * y + offset_y, d_tile * (y + 1) + offset_y),
                )
            ]
    tiles = tuple(tiles)
    if buffer is None and buffer_fraction is None:
        ...
        for tile in tiles:
            buffered_tiles += [
                (
                    (tile[0][0] - buffer_pixels, tile[0][1] + buffer_pixels),
                    (tile[1][0] - buffer_pixels, tile[1][1] + buffer_pixels),
                )
            ]
