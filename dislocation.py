...
def deform_point_cloud(
    point_cloud, strike, dip, d=0.01, u_ss=1.0, u_ds=1.0, x0=None, y0=None
):
    ...
    xyz = (
        get_xyz_from_pdal(read_file(point_cloud))
        if isinstance(point_cloud, str)
        else get_xyz_from_pdal(point_cloud[0])
        if isinstance(point_cloud, list)
        else point_cloud
        if point_cloud.dtype.fields is None
        else get_xyz_from_pdal(point_cloud)
    )
    ...
    return (
        put_xyz_to_pdal(read_file(point_cloud), deformed_points)
        if isinstance(point_cloud, str)
        else list(put_xyz_to_pdal(point_cloud[0]))
        if isinstance(point_cloud, list)
        else put_xyz_to_pdal(point_cloud)
        if point_cloud.dtype.fields is not None
        else deformed_points
    )
